...
Companion documents, next to this one: `01-brief-movie-watchlist-tracker` and `02-tech-spec-movie-watchlist-tracker`. Figma: [Movie / Watchlist Tracker](https://www.figma.com/design/i6yFe3BHJaGOe8MSK9BtTK/Movie---Watchlist-Tracker?node-id=1-4), page "Screens".
...
5. **Track coverage** with the checklist at the end of this handout. Cross off each requirement ID once a task covers it. You're done when all 78 are crossed off.
...
|---|---|---|
| Highest | Nothing works without it | Saving a title (ADD-2, ADD-3): dashboard, library, genres, and Picker all feed off titles |
...
- **Description:**
  ...
  Figma: [11 · Delete confirmation](https://www.figma.com/design/i6yFe3BHJaGOe8MSK9BtTK/Movie---Watchlist-Tracker?node-id=3023-1434)
...
3. All 78 requirement IDs on the checklist are crossed off.
...
1. **Inventing features.** No sign-in, no video player, no genre editor, no mobile layout: the design has none of these. If it has no screen and no requirement ID, it's not a task. Park it for the designer.
...
Print this, cross off each ID when a task covers it. 78 in total.
