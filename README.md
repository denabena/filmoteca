...
Take a copy, build your project in it. It is deliberately small: an app shell and one
backend endpoint, and the rest is yours.
...
The footer profile is deliberately mocked in one place,
[`frontend/src/lib/current-user.ts`](frontend/src/lib/current-user.ts), until the sign in
endpoint exists. That file is the only thing that changes when it does.

**One backend endpoint,** `GET /api/hello`, in
[`backend/src/app.controller.ts`](backend/src/app.controller.ts). It is short, so read it
first to see how a Nest controller and service fit together:
